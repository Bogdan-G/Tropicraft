...
public class WorldGenHomeTree extends TCGenBase {
	...
	private static final int woodMeta = 1;
	private final Block leafID = TCBlockRegistry.rainforestLeaves;
	private static final int leafMeta = 0;
...
/*		BiomeGenBase currentBiome = worldObj.getWorldChunkManager().getBiomeGenAt(i, k);
			...
			//if(rand.nextInt(6) == 0);//off? okay
			{
				if(y >= j + height - 6)
				{
					...
					for(int x = 0; x < branches; x++)
					{
						...
						int branchZ1 = trunkZ;
						double angle = rand.nextFloat() * (float)Math.PI * 2D;
						branchX1 = (int)((trunkRadius * org.bogdang.modifications.math.MathHelperLite.sin(angle)) + branchX1);
						branchZ1 = (int)((trunkRadius * org.bogdang.modifications.math.MathHelperLite.cos(angle)) + branchZ1); 
						int branchX2 = (int)((branchLength * org.bogdang.modifications.math.MathHelperLite.sin(angle)) + branchX1);
						int branchZ2 = (int)((branchLength * org.bogdang.modifications.math.MathHelperLite.cos(angle)) + branchZ1); 
	...
	public void generateBranches(int topX, int topZ, int height)
	{
		...
		/*for(int x = 0; x < topBranches; x++)
		{
			...
			int branchZ1 = topZ;
			double angle = rand.nextFloat() * (float)Math.PI * 2D;
			branchX1 = (int)((baseDistance * org.bogdang.modifications.math.MathHelperLite.sin(angle)) + branchX1);
			branchZ1 = (int)((baseDistance * org.bogdang.modifications.math.MathHelperLite.cos(angle)) + branchZ1); 
			int branchX2 = (int)((branchLength * org.bogdang.modifications.math.MathHelperLite.sin(angle)) + branchX1);
			int branchZ2 = (int)((branchLength * org.bogdang.modifications.math.MathHelperLite.cos(angle)) + branchZ1); 
